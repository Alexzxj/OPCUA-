...
class DBTrans():
    ...
    def FetChall(self, sqlString, params = None):
        try:
            ...
            Logger.log.error(sqlString + "," + str(e))
            raise e
    ...
    def ExistTable(self, tableName):
        returnData = False
        try:
            data = self.Cursor.FetChall("show tables like '%s'" % tableName)
    ...
    # 判断视图是否存在
    def ExistView(self, viewName):
        returnData = False
        try:
            data = self.FetChall("show table status where comment='view' and name = '%s'" % viewName)
            returnData = len(data) > 0
        except Exception as e:
            Logger.log.error(str(e))
            raise e
